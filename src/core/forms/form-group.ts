...
export class FormGroup<T extends Record<string, any>> {
  ...
  patchValue(value: Partial<T>, options: { emitEvent?: boolean } = {}): void {
    ...
    for (const key in value) {
      if (key in this.controls) {
        this.controls[key as keyof T].setValue(value[key as keyof T] as any, {
          emitEvent: false,
        });
      }
    }
  ...
  reset(value?: Partial<T>, options: { emitEvent?: boolean } = {}): void {
    ...
    for (const key in this.controls) {
      this.controls[key].reset(value ? (value[key] as any) : undefined, {
        emitEvent: false,
      });
    }
  ...
  private updateStatus(): void {
    if (Object.values(this.controls).some((control) => control.isPending())) {
      ...
      Object.values(this.controls).some((control) => control.isDisabled())
    ) {
      this.status = "DISABLED";
    } else if (
      Object.values(this.controls).some((control) => !control.isValid())
    ) {
      this.status = "INVALID";
    } else {
      this.status = "VALID";
